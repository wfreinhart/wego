...
import type { Plan, World, Vec2 } from '@engine/types';
...
export class PlanningOverlay {
  ...
  updateWorld(world: World, canvas: HTMLCanvasElement) {
    this.world = world;
    this.canvas = canvas;
  ...
  clearPlan() {
    this.plan = { unitId: 1, waypoints: [] };
  }

  draw() {
    this.container.removeChildren();
    if (!this.enabled || !this.planning || !this.world) {
      return;
    }
